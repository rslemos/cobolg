...
public class CompilerStatementsProcessor {
	private static final Logger logger = LoggerFactory.getLogger(CompilerStatementsProcessor.class);

	public void injectCompilerStatements(CompilerStatementsContext preTree, ParserRuleContext mainTree) {
	...
	private void showInput(List<CompilerStatementContext> statements) {
		for (CompilerStatementContext stmt : statements)
			logger.debug(format("[%8s]: %s", stmt.getSourceInterval(), stmt.toStringTree(Arrays.asList(COBOLParser.ruleNames))).toString());
	}
	...
	private void showReturn(List<CompilerStatementContext> statements, List<Neighbor<TerminalNode>> result) {
		...
		for (int i = 0; i < statements.size(); i++) {
			...
			logger.debug(format("[%8s] [%8s] [%8s] - %s %s %s",
					...
					neighbor.left, stmt.toStringTree(Arrays.asList(COBOLParser.ruleNames)), neighbor.right));
		}
	}
